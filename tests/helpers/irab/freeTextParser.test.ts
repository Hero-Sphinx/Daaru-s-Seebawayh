 ...
 * tests/fixtures/irab/camelCandidates.json (npm run irab:record-fixtures), so the
...
function parse(text: string) {
  ...
  for (const w of words) if (!(w in FIXTURES)) throw new Error(`No fixture for "${w}" — add the sentence to tests/fixtures/irab/sentences.ts and re-record`);
