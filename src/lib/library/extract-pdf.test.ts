...
import { normalizePdfText } from "./extract-pdf";
...
describe("ocrPages", async () => {
  const { ocrPages, OCR_BATCH_SIZE } = await import("./extract-pdf");
  const { GeminiQuotaExhaustedError } = await import("@/lib/gemini-client");
