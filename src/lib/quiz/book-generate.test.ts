...
import { mulberry32 } from "@/lib/quiz/primitives";
import { buildFawaidQuestions, buildIrabExcerptQuestions, selectSentencesForMeaningMatch, splitIntoSentences, type FawaidPoolItem } from "./book-generate";
import type { RawCandidate } from "@/lib/irab/free-text-parser";
import camelFixtures from "@/lib/irab/__fixtures__/camel-candidates.json";
...
describe("buildIrabExcerptQuestions", () => {
  it("produces questions for a fully-resolvable diacritized sentence", async () => {
    // Real CAMeL analyses (src/lib/irab/__fixtures__, recorded by npm run irab:record-fixtures).
