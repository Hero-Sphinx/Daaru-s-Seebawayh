...
import { wisdomIndexForDate } from "@/lib/data/wisdom";
import { realName } from "@/lib/display-name";
import { TIMEZONE_COOKIE } from "@/lib/theme-prefs";
...
export default async function DashboardPage() {
  ...
  const [cardsDueToday, wordsMastered, totalAttempts7d, correctAttempts7d, documentsInLibrary, recentDocuments, recentFawaid, me] = await Promise.all([
    ...
    db.quiz_attempts.count({ where: { user_id: userId, answered_at: { gte: daysAgo(7) } } }),
    db.quiz_attempts.count({
      where: { user_id: userId, answered_at: { gte: daysAgo(7) }, is_correct: true },
    }),
  ...
  const name = realName(me?.display_name);
  const rawTz = (await cookies()).get(TIMEZONE_COOKIE)?.value ?? "UTC";
  let timeZone = "UTC";
  try {
    timeZone = decodeURIComponent(rawTz);
  } catch {}
  const quizAccuracyLabel = totalAttempts7d > 0 ? `${Math.round((correctAttempts7d / totalAttempts7d) * 100)}%` : "—";
  ...
  return (
    <div className="space-y-8">
      ...
      <WisdomCard index={wisdomIndexForDate(new Date(), timeZone)} serverTimeZone={timeZone} />
